	...
	public static String encode(String str) {
		...
		for (String strAlpha : list) {
			...
			if(Character.isUpperCase(charAlpha)) {//大文字なら
				intAlpha = ((intAlpha == CODE_UPPER_Z) ? CODE_UPPER_A : intAlpha + 1);//ZならAの文字コード、それ以外は現在の文字コードから1を足したものを代入
			}else {//小文字なら
				intAlpha = ((intAlpha == CODE_LOWWER_Z) ? CODE_LOWWER_A : intAlpha + 1);//zならaの文字コード、それ以外は現在の文字コードから1を足したものを代入
			}
			String convertString = Character.toString(intAlpha);//int型を対応する文字列に変換
			encodeSb.append(convertString);//連結
		}
